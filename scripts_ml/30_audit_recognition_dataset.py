...
JSON_OUT = REPORT_DIR / "recognition_audit_words.json"
CSV_OUT = REPORT_DIR / "recognition_audit_words.csv"

MIN_SEQS = int(os.environ.get("VOXGEST_MIN_SEQS_PER_CLASS", "80"))
MIN_GROUPS = int(os.environ.get("VOXGEST_MIN_GROUPS_PER_CLASS", "4"))
FOCUS_WORDS = {"WATER", "THANKYOU", "YES", "NO", "DOCTOR", "PLEASE", "HELLO"}
REQUIRED_LABELS = list(DEMO10_WORDS) + list(NEGATIVE_WORDS)
...
def audit_label(label, metadata):
    ...
    for file_path in sorted(label_dir.glob("*.npy")):
        ...
        if not has_hand_policy_metadata(label, file_path.name, metadata):
...
def main():
    ...
    print("=" * 76)
    print("VoxGest Recognition Hardening v1 dataset audit")
    print("=" * 76)
    ...
    print()
    for row in rows:
        status = "OK" if row["ready"] else "FIX"
        print(
            f"{status:>3} {row['label']:<10} seq={row['valid_shape']:>4} "
