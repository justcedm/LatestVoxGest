...
SAVE_DIR = Path(
    os.environ.get(
        "VOXGEST_RECORDER_OUTPUT_ROOT",
        os.environ.get("VOXGEST_LSTM_DATASET", ROOT / default_dataset_dir_name()),
    )
)
METADATA_PATH = SAVE_DIR / "metadata_lstm_v2.json"
SIGNER_ID = (
    os.environ.get("VOXGEST_SIGNER_ID", "TEAMMATE")
    .strip()
    .upper()
    .replace(" ", "_")
)
        ...
        if recording and vec is not None:
            ...
            if len(frame_buffer) == SEQ_LEN and frames_since_save >= STRIDE_FRAMES:
                ...
                if hand_frames >= MIN_HAND_FRAMES:
                    ...
                    file_name = f"{SIGNER_ID}_{word}_{saved + 1:03d}_{source_stamp}_seq{saved:04d}.npy"
