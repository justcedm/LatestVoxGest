...
Trains a Conv1D temporal classifier from the same 30x162 MediaPipe motion
features used by the LSTM pipeline. This keeps the motion-tracking contract
stable while giving us a faster, non-recurrent model to compare against LSTM.
"""
...
def default_training_data_dir():
    ...
    if WORD_PROFILE == "fullsign225_phrase_v1":
        return ROOT / "external_datasets" / "fullsign225_phrase_v1_features"
    if WORD_PROFILE == "onehand162_phrase_v1":
        return ROOT / "external_datasets" / "onehand162_phrase_v1_features"
...
def build_model(num_classes):
    filters = int(os.environ.get("VOXGEST_TCN_FILTERS", "96"))
    kernel_size = int(os.environ.get("VOXGEST_TCN_KERNEL", "3"))
    dropout = float(os.environ.get("VOXGEST_TCN_DROPOUT", "0.20"))
    ...
    return Model(inp, out, name="VoxGest_TCN_v1")
