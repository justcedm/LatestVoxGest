...
def draw(frame, model_name, expected, trial_count, last_row, last_prediction=None):
    h, w = frame.shape[:2]
    overlay = frame.copy()
    ...
    if last_prediction:
        allowed = "yes" if last_prediction.get("inference_allowed") else "no"
        cv2.putText(
            frame,
            (
                f"pred {last_prediction.get('label') or '-'} "
                f"conf {float(last_prediction.get('confidence', 0.0)):.0%} "
                f"hand {float(last_prediction.get('hand_presence', 0.0)):.0%} "
                f"quality {last_prediction.get('quality_status', '-')}"
            ),
            (15, 112),
        ...
        cv2.putText(
            frame,
            f"inference_allowed={allowed}",
            (15, 138),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.48,
            (160, 220, 160) if last_prediction.get("inference_allowed") else (120, 200, 255),
            1,
        )
    if last_row:
        cv2.putText(
...
def main():
    ...
    last_prediction = {
        ...
        "quality_reason": "waiting for a complete quality-gated sequence",
    ...
    with mp_holistic.Holistic(
        ...
        while cap.isOpened():
            ...
            if vec is not None:
                ...
                quality_window.append(
                    capture_frame_quality(results, mirrored_input=MIRROR_INPUT)
                )
            ...
            draw(frame, model_name, expected, trial_count, last_row, last_prediction)
            ...
            if key == ord("n"):
                ...
                failures = gate_failure(
                    ...
                    last_prediction.get("quality_status", GOOD),
                )
                ...
                row = {
                    ...
                    "quality_status": last_prediction.get("quality_status", GOOD),
                    "quality_reason": last_prediction.get("quality_reason", "ok"),
                    "inference_allowed": bool(last_prediction.get("inference_allowed", False)),
