"""Hands-free teammate webcam recorder for VoxGest FullSign225 samples.

This records 30-frame x 225-feature webcam sequences into a separate
portable recorder-pack dataset:

    recorded_features/fullsign225_manual5_team_features

It does not train models or write to demo10, onehand162, sprint30, or
fullsign225_team16.
"""
...
os.environ.setdefault("VOXGEST_WORD_PROFILE", "fullsign225_manual5_team")
...
SIGNER_ID = "".join(
    ...
    for ch in os.environ.get("VOXGEST_SIGNER_ID", "TEAMMATE_A").strip().upper()
) or "TEAMMATE_A"


def _configured_save_dir():
    raw = os.environ.get("VOXGEST_RECORDER_OUTPUT_ROOT") or os.environ.get(
...
def load_metadata():
    ...
    return {
        ...
        "source_dataset": "team_manual_webcam",
        "recorder_pack": True,
        "samples": {},
    }
...
def existing_count(word):
    ...
    if not word_dir.exists():
        return 0
    return sum(1 for path in word_dir.glob(f"{SIGNER_ID}_{word}_*.npy"))
...
def record_word(word, cap, holistic, draw_utils, metadata):
    ...
    accepted = 0
    attempts = 0
    source_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    source_id = f"{word}/{SIGNER_ID}_{word}_{source_stamp}"
    ...
    while accepted < TARGET_SEQUENCES_PER_WORD:
        ...
        if ok:
            file_name = f"{SIGNER_ID}_{word}_{existing + accepted + 1:03d}.npy"
...
def main():
    if any(arg in {"-h", "--help", "/?"} for arg in sys.argv[1:]):
        print("VoxGest FullSign225 Teammate Recorder")
        print()
        print("Usage:")
        print("  python scripts_ml\\41_record_manual_fullsign225_words.py EAT WATER HELLO THANKYOU")
        print("  python scripts_ml\\41_record_manual_fullsign225_words.py NOTHING")
        print()
        print("Environment:")
        print("  VOXGEST_SIGNER_ID=TEAMMATE_A")
        print("  VOXGEST_RECORDER_OUTPUT_ROOT=recorded_features\\fullsign225_manual5_team_features")
        print("  VOXGEST_MANUAL_SEQUENCES_PER_WORD=30")
        print()
        print("Keys in webcam window: Q/ESC quit, S skip current word")
        return
    ...
    print("=" * 72)
    print("VoxGest FullSign225 Teammate Webcam Recorder")
    print("=" * 72)
    ...
    print("\nDone. Zip the recorded_features folder and send it to Ced.")
