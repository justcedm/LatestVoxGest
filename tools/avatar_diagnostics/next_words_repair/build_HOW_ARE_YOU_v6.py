...
candidate={'label':'HOW ARE YOU','stem':str(root/'work/recovered_authoritative/SOURCE_HANDOFF/fsl105_avatar_landmarks/HOW ARE YOU/clips__4__0')}
fits=json.loads((out/'HOW_ARE_YOU_fit_v5.json').read_text())
...
for candidate in [candidate]:
 ...
 act=bpy.data.actions.new('FSL_HOW_ARE_YOU__SOURCE_FIT_V6');act.use_fake_user=True;act['listen_ready']=False;act['status']='EXPERIMENTAL_NOT_ACCEPTED';arm.animation_data.action=act
 ...
 bpy.ops.wm.save_as_mainfile(filepath=str(out/'HOW_ARE_YOU_SOURCE_FIT_v6.blend'))
 for frame in [132,172]:
  scene.frame_set(frame);scene.render.filepath=str(out/f'HOW_ARE_YOU_v6_{frame}.png');bpy.ops.render.render(write_still=True)
 (out/'HOW_ARE_YOU_v6_measurements.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
