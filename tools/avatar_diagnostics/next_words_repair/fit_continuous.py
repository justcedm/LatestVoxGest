...
for side,col,lo in [('R',2,54),('L',1,33)]:
 ...
 for sign in [1,-1]:
  for flex in [10,65]:
   ...
   fit=least_squares(lambda z:np.r_[(calc(z,sign)[1:]-obs[anchor,1:]).ravel(),.012*z[:17]],start,bounds=(low,high),loss='soft_l1',f_scale=.1,max_nfev=100)
 ...
 for seq in [[i for i in grid if i>anchor],list(reversed([i for i in grid if i<anchor]))]:
  ...
  for i in seq:
   ...
   def fun(z):return np.r_[(calc(z,sign)[1:]-obs[i,1:]).ravel(),.12*(z[:17]-prev[:17]),.5*(z[18:21]-prev[18:21]),.035*(z[17]-prev[17])]
...
(out/(tag+'_fit_v4.json')).write_text(json.dumps(result))
