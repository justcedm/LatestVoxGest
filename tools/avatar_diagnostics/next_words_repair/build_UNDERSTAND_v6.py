...
candidate={'label':'UNDERSTAND','stem':str(root/'work/recovered_authoritative/SOURCE_HANDOFF/fsl105_avatar_landmarks/UNDERSTAND/clips__10__0')}
fits=json.loads((out/'UNDERSTAND_fit_v4.json').read_text())
...
for candidate in [candidate]:
 ...
 act=bpy.data.actions.new('FSL_UNDERSTAND__SOURCE_FIT_V6');act.use_fake_user=True;act['listen_ready']=False;act['status']='EXPERIMENTAL_NOT_ACCEPTED';arm.animation_data.action=act
 ...
 for fi in range(len(raw)):
  ...
  for side,sh,el,wr in [('L',11,13,15),('R',12,14,16)]:
   ...
   # Match the observed index-tip position relative to the eyes, adapting head proportions.
   if side=='R':
    hm=desired[hn].to_matrix().to_4x4()
    for seg in range(3):
     n=f'DEF-f_index.{seg+1:02d}.R';pn=hn if seg==0 else f'DEF-f_index.{seg:02d}.R';qq=fit['finger_local_xyzw'][n][j];hm=hm@arm.data.bones[pn].matrix_local.inverted()@arm.data.bones[n].matrix_local@Quaternion((qq[3],qq[0],qq[1],qq[2])).to_matrix().to_4x4()
    tip=hm@Vector((0,arm.data.bones['DEF-f_index.03.R'].length,0));eyes=(neutral_heads['EyeBall_L']+neutral_heads['EyeBall_R'])/2;sourceeyes=(pose[fi,2]+pose[fi,5])/2;facscale=(neutral_heads['EyeBall_L']-neutral_heads['EyeBall_R']).length/max(abs(pose[fi,2,0]-pose[fi,5,0]),1e-5)
    source_tip=hands[side][fi,8];face_target=eyes+Vector((source_tip-sourceeyes)*facscale);target.x=face_target.x-tip.x;target.z=face_target.z-tip.z
 ...
 bpy.ops.wm.save_as_mainfile(filepath=str(out/'UNDERSTAND_SOURCE_FIT_v6.blend'))
 for frame in [132,172]:
  scene.frame_set(frame);scene.render.filepath=str(out/f'UNDERSTAND_v6_{frame}.png');bpy.ops.render.render(write_still=True)
 (out/'UNDERSTAND_v6_measurements.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
